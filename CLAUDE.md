...
- **The agent lives in a bottom dock, not a rail tab.** The Review page's right rail is file-tree only; the agent panel is a resizable bottom dock spanning under the diff and file tree, never under the PR list (`DESIGN_SPEC_REVIEW.md` §5, "The agent dock"). The reasoning is simultaneity, not aesthetics: a reviewer never needs the PR list and the chat at once, but does need the file tree and the chat at once, and the rail-tab design made that impossible. The dock collapses to a **44px identity strip and never to zero** — it's the only place the outage and `CACHED` banners live, so collapsing it to nothing would silently hide an outage.
