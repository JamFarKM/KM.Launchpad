...
import { duration, runTone, timeAgo } from "../lib/format";
...
export function PipelineCard({ item, onRun, onOpenRun, onRemove }: Props) {
  ...
  return (
    <div className="card">
      ...
      <div className="runs">
        {runsQ.isLoading && (
          <div className="faint" style={{ fontSize: 12 }}>
            <span className="spin" /> loading runs…
          </div>
        )}
        ...
        {runs.map((r) => (
          <div
            ...
            title="View run & logs"
          >
            <span className={`badge ${runTone(r)}`} style={{ minWidth: 0 }}>
              <span className="dot" />
            </span>
      ...
      <div className="actions">
        <button className="btn primary small" onClick={() => onRun(item)}>
          ▶ Run
        </button>
        {latest && (
          <button className="btn small" onClick={() => onOpenRun(item.project, latest.id)}>
            Logs
          </button>
        )}
        <span style={{ flex: 1 }} />
        <button className="btn ghost small" title="Remove from view" onClick={() => onRemove(item)}>
          ✕
        </button>
      </div>
    </div>
